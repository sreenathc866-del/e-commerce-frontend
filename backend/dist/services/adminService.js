...
class AdminService {
    ...
    async getDashboardStats() {
        ...
        const { data: transactions, error: txError } = await supabase_1.supabaseAdmin
            .from('transactions')
            .select('*')
        ...
        const successRate = (successCount + failedCount) > 0
            ...
            : 0;
        // 2. Commission
        const { data: orderItems, error: itemsError } = await supabase_1.supabaseAdmin
            .from('order_items')
        ...
        const { data: transfers, error: tfError } = await supabase_1.supabaseAdmin
        ...
        if (!tfError && transfers) {
            transfers.forEach(tf => {
                if (tf.status === 'processed')
                    vendorPayouts += Number(tf.amount);
                else
                    pendingTransfers += Number(tf.amount);
            });
        }
        // 4. Vendors Count
        const { count: vendorCount } = await supabase_1.supabaseAdmin
            .from('vendors')
